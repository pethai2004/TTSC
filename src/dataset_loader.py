...
def load_ljspeech(target_dir='./assets/LJSpeech-1.1', create_meta_json="formatted", add_root_path_key=True, preprocess_metadata=True):
    ...
    path_to_data = os.path.join(os.getcwd(), target_dir)
    os.makedirs(path_to_data, exist_ok=True)
    
    if not os.path.exists(path_to_data): # assume the dataset is not loaded
        ...
        with tarfile.open(file_path, 'r:bz2') as t:
            ...
            try:
                shutil.move('LJSpeech-1.1', destination)
            except shutil.Error:
                logger.info(f"------->>>> Dataset already exists at {path_to_data}")
                
            os.remove(file_path)
            logger.info(f"------->>>> Finished loading dataset path: {target_dir}")
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target_dir", type=str, default='./assets/LJSpeech-1.1')
